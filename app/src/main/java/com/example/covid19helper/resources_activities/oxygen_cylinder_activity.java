...
public class oxygen_cylinder_activity extends AppCompatActivity implements itemAdapter.OnItemHoldListener,itemAdapter.OnItemClickListener {
            ...
            if (snapshot.exists()){
                ...
                for(DataSnapshot snapshot_item: snapshot.getChildren()){
                    if(snapshot_item.getValue().equals(muser.getUid())){
                        Toast.makeText(oxygen_cylinder_activity.this,"Your Feedback for this lead is already been recorded",Toast.LENGTH_SHORT).show();
                        flag = true;
                    }
                }
                if(!flag){

                        Intent intent = new Intent(oxygen_cylinder_activity.this, feedback.class);
                        intent.putExtra("post_id",post_id);
                        intent.putExtra("user_uid", muser.getUid());
                        intent.putExtra("calling_activity",oxygen_cylinder_activity.class.toString());
